...
class ProjetController extends Controller
{
    ...
    //     $porteurProjetConnecte = Auth::guard('porteurprojet')->user();
    //     $idPersonneConnectee = $porteurProjetConnecte->id;
    //     dd($idPersonneConnectee);
        
    //     $donneeProjetValide['is_deleted']=0;
    //     $donneeProjetValide['porteurprojet_id']=$idPersonneConnectee;
    ...
    public function store(StoreProjetRequest $request)
    {
        try {
            ...
            // Vérifier si l'utilisateur porteur de projet est authentifié
            if (Auth::guard('porteurprojet')->check()) {
                $porteurProjetConnecte = Auth::guard('porteurprojet')->user();
                $idPersonneConnectee = $porteurProjetConnecte->id;
            } else {
                return response()->json([
                    "status" => 0,
                    "messageErreur" => "Utilisateur porteur de projet non authentifié",
                ]);
            }
            ...
            if ($projet->save()) {
                ...
                return response()->json([
                    "status" => 0,
                    "message" => "Le projet n'a pas été enregistré"
                ]);
            }
        } catch (\Throwable $th) {
            return response()->json([
